...
export async function createSearchablePDF(file: File, options: CreateSearchablePDFOptions = {}): Promise<Uint8Array> {
  const {
    language = "eng",
  ...
  const workers = await Promise.all(
    Array(numWorkers)
      .fill(null)
      .map(() => Tesseract.createWorker(language, 1)),
  );
  ...
  try {
    if (numWorkers === 1) {
      ...
      const processPage = async (pageIndex: number, workerIndex: number) => {
        ...
        completed++;
        onProgress?.(
          Math.round(25 + (completed / images.length) * 50),
          `Running OCR on page ${completed} of ${images.length}...`,
        );
      };
 ...
 */
async function extractOCRWordsWithWorker(worker: Tesseract.Worker, imageBlob: Blob): Promise<OCRWord[]> {
  ...
  if (data.hocr) {
    const wordRegex =
      /<span class='ocrx_word'[^>]*title='bbox (\d+) (\d+) (\d+) (\d+); x_wconf (\d+)'[^>]*>([^<]+)<\/span>/g;
    for (const match of data.hocr.matchAll(wordRegex)) {
