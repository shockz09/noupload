...
//
// Audio: has to be re-timed sample by sample. Scaling audio packet timestamps alone
// would leave gaps (slower) or overlaps (faster) that players resync away, so audio
// is decoded to PCM, WSOLA time-stretched so the pitch survives (or linearly
// resampled for the tape effect, when preservePitch is off) and re-encoded to AAC.

import type { InputAudioTrack, InputVideoTrack, Mp4OutputFormat } from "mediabunny";
import { createTimeStretcher } from "../audio/time-stretch";
...
export interface SpeedOptions {
   ...
   * tape effect, where speeding up raises pitch.
   */
