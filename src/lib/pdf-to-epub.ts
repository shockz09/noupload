...
async function resolveOutline(
  ...
  for (const item of items) {
    ...
    const children = item.items && item.items.length > 0 ? await resolveOutline(pdf, item.items) : [];
...
function buildChaptersFromBookmarks(lines: ClassifiedLine[], bookmarks: BookmarkNode[]): Chapter[] {
...
function buildChapters(lines: ClassifiedLine[], bookmarks: BookmarkNode[]): Chapter[] {
  if (lines.length === 0) {
...
function buildChaptersByLevel(lines: ClassifiedLine[], splitLevel: HeadingLevel): Chapter[] {
  ...
  return chapters.length > 0 ? chapters : [{ title: "Document", html: "<p>No content.</p>" }];
}
...
function buildChaptersByGroupedPages(lines: ClassifiedLine[]): Chapter[] {
  ...
  for (let i = 0; i < pageNumbers.length; i += chunkSize) {
    ...
    if (html.trim()) {
      const title = startPage === endPage ? `Page ${startPage}` : `Pages ${startPage}–${endPage}`;
...
export async function convertPdfToEpub(file: File, onProgress?: (progress: number) => void): Promise<ConversionResult> {
  // Step 1: Extract text items + bookmarks (0-60%)
  onProgress?.(5);
  const { items, pageCount, pageDimensions, bookmarks, metadata } = await extractTextItems(file, (current, total) => {
