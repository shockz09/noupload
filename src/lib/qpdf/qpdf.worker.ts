...
import type { QpdfCompressOptions, QpdfEncryptOptions, QpdfWorkerMessage, QpdfWorkerResponse } from "./types";
...
function buildEncryptArgs(options: QpdfEncryptOptions): string[] {
  const args = ["--encrypt", options.userPassword || "", options.ownerPassword, options.keyLength.toString()];
  ...
  try {
    ...
    switch (operation) {
      ...
      case "decrypt": {
        ...
        args = [`--password=${decryptOpts.password}`, "--decrypt", INPUT_PATH, OUTPUT_PATH];
        break;
      }
