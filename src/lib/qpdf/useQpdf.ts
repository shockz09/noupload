...
function getWorker(): Worker {
  if (!workerInstance) {
    workerInstance = new Worker(
      new URL('./qpdf.worker.ts', import.meta.url),
      { type: 'module' }
    );
  }
  workerRefCount++;
...
export function useQpdf() {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent<QpdfWorkerResponse>) => {
      ...
      if (success && data) {
        ...
        const errorMsg = errorCode ? ERROR_MESSAGES[errorCode] : error || 'Unknown error';
  ...
  const sendOperation = useCallback(async (
    ...
    setState(s => ({ ...s, isProcessing: true, progress: 10, error: null }));
    ...
    return new Promise((resolve, reject) => {
      ...
      // Send to worker
      const message: QpdfWorkerMessage = { id, operation, inputData, options };
  ...
  const repair = useCallback(async (file: File): Promise<Uint8Array> => {
    return sendOperation('repair', file);
  }, [sendOperation]);

  const linearize = useCallback(async (file: File): Promise<Uint8Array> => {
