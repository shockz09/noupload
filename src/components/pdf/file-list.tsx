...
export const FileList = memo(function FileList({ files, onRemove, onReorder, onClear }: FileListProps) {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, toIndex: number, targetId: string) => {
      e.preventDefault();
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="file-number">{files.length}</div>
      ...
      {onReorder && files.length > 1 && (
        <p className="text-xs text-muted-foreground text-center font-medium pt-2">Drag files to reorder them</p>
      )}
    </div>
  );
});
