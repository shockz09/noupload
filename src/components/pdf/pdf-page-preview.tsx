  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white border-2 border-foreground overflow-hidden">
        {/* Page number badge */}
        <div className="absolute top-2 left-2 z-10 file-number text-xs">{page.pageNumber}</div>
        ...
        {overlay && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">{overlay}</div>
        )}

        {/* Hover effect */}
        {onClick && <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/10 transition-colors" />}
      </div>
...
export const PageGridLoading = memo(function PageGridLoading({ progress }: { progress: number }) {
  return (
