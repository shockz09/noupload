...
export const ProgressBar = memo(function ProgressBar({ progress, label }: ProgressBarProps) {
...
export const AudioFileInfo = memo(function AudioFileInfo({ file, duration, onClear, icon }: AudioFileInfoProps) {
  const sizeText =
    ...
    <div className="p-4 border-2 border-foreground bg-card space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {filename && state === "extracting" && <p className="text-xs text-muted-foreground truncate">{filename}</p>}
        </div>
      </div>
