...
export const AudioPlayer = memo(function AudioPlayer({ src, className = "" }: AudioPlayerProps) {
  ...
  const formatTime = useCallback((seconds: number) => formatDuration(Number.isFinite(seconds) ? seconds : 0), []);
  ...
  const handleProgressClick = useCallback((e: React.MouseEvent) => seekTo(e.clientX), [seekTo]);
        ...
        <div className="flex items-stretch">
          ...
          >
            ...
            {isPlaying ? (
              ...
              // Play icon - triangle
              <svg aria-hidden="true" className="w-7 h-7 ml-1 relative z-10" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className="flex-1 flex flex-col">
            ...
            >
              ...
              <div className="absolute inset-y-0 left-0 overflow-hidden" style={{ width: `${progress}%` }}>
                ...
                >
                  {WAVEFORM_INDICES.map((i) => (
                    <div key={i} className="flex-1 bg-foreground/70" style={{ height: `${getBarHeight(i) * 80}%` }} />
                  ))}
                </div>
              </div>
