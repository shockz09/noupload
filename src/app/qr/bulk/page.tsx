  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        for (const text of items.slice(0, 20)) { // Limit to 20 previews
          let dataUrl: string;
          if (logo) {
            ...
            dataUrl = await generateQRDataURL(text.trim(), { width: 200, ...colorOptions });
          }
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
                  ...
                  {previews.map((preview, index) => (
                    <div key={index} className="space-y-1 group relative">
                      <div className="bg-white p-2 border border-foreground/20 relative">
                        <img src={preview.dataUrl} alt={`QR ${index + 1}`} className="w-full" />
          ...
          {previews.length > 0 && (
            ...
            >
              {isDownloading ? (
                <><LoaderIcon className="w-5 h-5" />Downloading...</>
              ) : (
                <><DownloadIcon className="w-5 h-5" />Download All ({items.length} files)</>
              )}
            </button>
          )}
        </div>
