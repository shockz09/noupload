...
import { DeletePagesIcon, PdfIcon, DownloadIcon, LoaderIcon } from "@/components/icons";
...
export default function DeletePage() {
  ...
  const hasChanges = deletedCount > 0 || rotatedCount > 0 || pageItems.some((p, i) => !p.deleted && p.pageNumber !== i + 1);
          ...
          {pagesLoading ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between p-3 border-2 border-foreground/30 bg-muted/30">
                ...
                {hasChanges && (
                  <button
                    onClick={() => setPageItems(pages.map((p) => ({ id: `page-${p.pageNumber}`, pageNumber: p.pageNumber, deleted: false, rotation: 0 })))}
                  ...
                  {pageItems.map((item, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {/* Bottom bar */}
                        <div className={`absolute bottom-0 left-0 right-0 text-white text-xs py-1 px-2 flex items-center justify-between ${
                          item.deleted ? "bg-red-500/70" : "bg-black/70"
                        }`}>
                          <span className="font-mono">{item.deleted ? "×" : index + 1 - pageItems.slice(0, index).filter(p => p.deleted).length}</span>
              ...
              >
                {isProcessing ? (
                  ...
                  <><XIcon className="w-5 h-5" />Cannot delete all pages</>
                ) : (
                  <><DownloadIcon className="w-5 h-5" />Download PDF ({remainingCount} pages)</>
                )}
              </button>
