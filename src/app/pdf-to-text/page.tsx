...
import { TextIcon, PdfIcon, DownloadIcon } from "@/components/icons";
    ...
    <div className="page-enter max-w-2xl mx-auto space-y-8">
      ...
      {result ? (
        <div className="space-y-6">
          <div className="card-base p-6 space-y-4">
            ...
            <div className="border-2 border-foreground/20 p-4 max-h-96 overflow-y-auto bg-muted/30">
              <pre className="text-sm whitespace-pre-wrap font-mono">{result.text.slice(0, 5000)}{result.text.length > 5000 && "\n\n... (truncated preview, download for full text)"}</pre>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <button onClick={handleExtract} disabled={isProcessing} className="btn-primary w-full">
            {isProcessing ? (
              <><span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Extracting...</>
            ) : (
              <><TextIcon className="w-5 h-5" />Extract Text</>
            )}
          </button>
        </div>
      )}
