...
export const Route = createFileRoute("/page-numbers")({
	head: () => ({
		meta: [
			{ title: "Add Page Numbers to PDF Free - Customize Position & Format | noupload" },
			{ name: "description", content: "Add page numbers to PDF for free. Customize position, format, and font size. Drag to place anywhere. Works 100% in your browser." },
			{ name: "keywords", content: "add page numbers pdf, pdf page numbers, number pdf pages, pdf pagination, insert page numbers" },
			{ property: "og:title", content: "Add Page Numbers to PDF Free" },
			{ property: "og:description", content: "Add customizable page numbers to your PDF. Drag to position anywhere. Works 100% offline." },
		],
	}),
...
function PageNumbersPage() {
  ...
  const formatPageNumber = useCallback(
    (pageNum: number, total: number) => {
      return format.replace("{n}", String(pageNum + startFrom - 1)).replace("{total}", String(total));
    },
    [format, startFrom],
  );
      ...
      {result ? (
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              >
                ...
                {/* Page Number Overlay */}
                <div className="absolute pointer-events-none transition-all duration-75" style={positionStyle}>
                  <span className="px-2 py-0.5 bg-white/90 border border-foreground/20 font-mono" style={fontSizeStyle}>
          ...
          <div className="space-y-6">
            ...
            {/* Action Button */}
            <button type="button" onClick={handleAddPageNumbers} disabled={isProcessing} className="btn-primary w-full">
              {isProcessing ? (
                <>
