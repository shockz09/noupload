...
export const Route = createFileRoute("/audio/remove-silence")({
	head: () => ({
		meta: [
			{ title: "Remove Silence from Audio Free - Trim Silent Parts | noupload" },
			{ name: "description", content: "Remove silence from audio for free. Automatically detect and trim silent portions. Works offline, completely private." },
			{ name: "keywords", content: "remove silence, trim silence, audio silence removal, cut silent parts, silence trimmer, free silence remover" },
			{ property: "og:title", content: "Remove Silence from Audio Free - Trim Silent Parts" },
			{ property: "og:description", content: "Remove silence from audio automatically for free. Works 100% offline." },
		],
	}),
	component: RemoveSilencePage,
});
...
function RemoveSilencePage() {
  ...
  const processFile = useCallback(
    async (fileToProcess: File, silenceMode: SilenceMode, thresh: number, minDur: number) => {
        ...
        <div className="space-y-4">
          ...
          <div className="border-2 border-foreground p-4 bg-card space-y-4">
            ...
            <div className="space-y-2 pt-2 border-t border-foreground/10">
              <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Silence Threshold</span>
