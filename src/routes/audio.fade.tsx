...
export const Route = createFileRoute("/audio/fade")({
	head: () => ({
		meta: [
			{ title: "Audio Fade In/Out Free - Add Fade Effects | noupload" },
			{ name: "description", content: "Add fade in and fade out effects to audio for free. Smooth audio transitions. Customizable duration. Works offline, completely private." },
			{ name: "keywords", content: "audio fade, fade in audio, fade out audio, audio transition, fade effect, free audio fade" },
			{ property: "og:title", content: "Audio Fade In/Out Free - Add Fade Effects" },
			{ property: "og:description", content: "Add fade in and fade out effects for free. Works 100% offline." },
		],
	}),
	component: FadeAudioPage,
});
...
import { applyFade, getAudioInfo } from "@/lib/audio-utils";
import { AUDIO_VIDEO_EXTENSIONS } from "@/lib/constants";
import { getErrorMessage } from "@/lib/error";
import { getFileBaseName } from "@/lib/utils";

function FadeAudioPage() {
    ...
    <div className="page-enter max-w-2xl mx-auto space-y-8">
      ...
      {result ? (
        <AudioResultView
          ...
          title="Fade Applied!"
          subtitle={`Fade in: ${usedFadeIn}s • Fade out: ${usedFadeOut}s`}
