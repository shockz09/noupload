...
export const Route = createFileRoute("/qr/generate")({
	head: () => ({
		meta: [
			{ title: "QR Code Generator Free - Create QR Codes Online | noupload" },
			{ name: "description", content: "Generate QR codes for free. Create QR codes for URLs, text, WiFi, email, phone, and more. Customizable colors and styles. Works offline, completely private." },
			{ name: "keywords", content: "qr code generator, create qr code, qr maker, free qr code, qr code creator, custom qr code" },
			{ property: "og:title", content: "QR Code Generator Free - Create QR Codes Online" },
			{ property: "og:description", content: "Generate QR codes for URLs, text, WiFi and more for free. Works 100% offline." },
		],
	}),
...
function QRGeneratePage() {
  ...
  const getDataString = useCallback((): string => {
    switch (dataType) {
      ...
      case "upi":
        return generateUpiString(upiData);
      default:
        return "";
    }
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (logo) {
          ...
          dataUrl = await generateQRDataURL(data, {
            width: 400,
            ...colorOptions,
          });
        }
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 text-center border-2 border-foreground transition-all ${
                        dataType === type.value ? "bg-foreground text-background" : "bg-card hover:bg-muted"
                      }`}
                    >
            ...
            <div className="space-y-4">
              {dataType === "text" && (
                <div className="space-y-2">
                  <label htmlFor="qr-text" className="input-label">
                    Your Text
                  </label>
                  <textarea
                    id="qr-text"
              ...
              {dataType === "url" && (
                <div className="space-y-2">
                  <label htmlFor="qr-url" className="input-label">
                    Website URL
                  </label>
                  <input
                    id="qr-url"
              ...
              {dataType === "email" && (
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label htmlFor="email-to" className="input-label">
                      Email Address
                    </label>
                    <input
                      id="email-to"
              ...
              {dataType === "sms" && (
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label htmlFor="sms-phone" className="input-label">
                      Phone Number
                    </label>
                    <input
                      id="sms-phone"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="sms-message" className="input-label">
                      Message (optional)
                    </label>
                    <textarea
                      id="sms-message"
              ...
              {dataType === "upi" && (
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label htmlFor="upi-vpa" className="input-label">
                      UPI ID (VPA) *
                    </label>
                    <input
                      id="upi-vpa"
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label htmlFor="upi-amount" className="input-label">
                        Amount (optional)
                      </label>
                      <input
                        id="upi-amount"
                    ...
                    <div className="space-y-2">
                      <label htmlFor="upi-note" className="input-label">
                        Note (optional)
                      </label>
                      <input
                        id="upi-note"
