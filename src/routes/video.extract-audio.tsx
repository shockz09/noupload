...
export const Route = createFileRoute("/video/extract-audio")({
	head: () => ({
		meta: [
			{ title: "Extract Audio from Video Free - Get Audio Track | noupload" },
			{ name: "description", content: "Extract audio from video files for free. Get the audio track from MP4, WebM, MOV. Works offline, completely private." },
			{ name: "keywords", content: "extract audio, video to audio, get audio from video, extract sound, free audio extractor" },
			{ property: "og:title", content: "Extract Audio from Video Free - Get Audio Track" },
			{ property: "og:description", content: "Extract audio from video files for free. Works 100% offline." },
		],
	}),
	component: ExtractAudioPage,
});
