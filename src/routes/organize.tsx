...
export const Route = createFileRoute("/organize")({
	head: () => ({
		meta: [
			{ title: "Organize PDF Pages Free - Rearrange, Reorder & Delete | noupload" },
			{ name: "description", content: "Rearrange PDF pages for free. Drag and drop to reorder, delete unwanted pages. Works 100% in your browser with complete privacy." },
			{ name: "keywords", content: "organize pdf, rearrange pdf pages, reorder pdf, delete pdf pages, pdf page order, rearrange pages" },
			{ property: "og:title", content: "Organize PDF Pages Free - Rearrange & Reorder" },
			{ property: "og:description", content: "Drag and drop to rearrange PDF pages. Delete unwanted pages. Works 100% offline." },
		],
	}),
...
import { addPageNumbers, organizePDF } from "@/lib/pdf-utils";
...
function OrganizePage() {
  ...
  const [numberPosition, setNumberPosition] = useState<"bottom-center" | "bottom-left" | "bottom-right" | "top-center">(
    "bottom-center",
  );
  ...
  const handleSelectAll = useCallback(() => {
    setSelectedPages((prev) => {
      ...
      return prev.size === pageItems.length ? new Set<number>() : new Set(pageItems.map((p) => p.id));
    });
  }, [pageItems]);
  ...
  const handleOrganize = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Failed to organize PDF"));
    } finally {
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-card border-2 border-foreground">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-medium">{pageItems.length} pages</span>
            </div>
              ...
              {pageItems.map((item, index) => (
                ...
                >
                  ...
                  <div className="relative bg-white border-2 border-foreground overflow-hidden">
                    ...
                    {/* Page number badge */}
                    <div className="absolute top-2 right-2 z-10 file-number text-xs">{item.id}</div>
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold border-2 border-foreground transition-colors ${
                          numberPosition === pos.value ? "bg-primary text-white" : "bg-muted hover:bg-accent"
                        }`}
                      >
                        {pos.label}
                      </button>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold border-2 border-foreground transition-colors ${
                          numberFormat === fmt.value ? "bg-primary text-white" : "bg-muted hover:bg-accent"
                        }`}
                      >
                        {fmt.label}
                      </button>
