...
export const Route = createFileRoute("/image/favicon")({
	head: () => ({
		meta: [
			{ title: "Favicon Generator Free - Create ICO Files Online | noupload" },
			{ name: "description", content: "Generate favicons for free. Convert images to ICO format. Create multi-size favicon packages. Works offline, your files stay private." },
			{ name: "keywords", content: "favicon generator, create favicon, ico converter, favicon maker, website icon, free favicon generator" },
			{ property: "og:title", content: "Favicon Generator Free - Create ICO Files Online" },
			{ property: "og:description", content: "Generate favicons from images for free. Works 100% offline." },
		],
	}),
...
import { FaviconIcon, ImageIcon } from "@/components/icons/image";
...
function FaviconPage() {
  ...
  const handleDownloadPng = useCallback(
    (key: keyof FaviconSet, filename: string) => {
      if (result) {
...
<link rel="apple-touch-icon" href="/apple-touch-icon.png">`}
        ...
        <div className="space-y-6">
          ...
          <button type="button" onClick={handleGenerate} disabled={isProcessing} className="btn-primary w-full">
            {isProcessing ? (
              <>
