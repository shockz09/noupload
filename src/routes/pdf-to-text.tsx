...
export const Route = createFileRoute("/pdf-to-text")({
	head: () => ({
		meta: [
			{ title: "PDF to Text Free - Extract Text from PDF Online | noupload" },
			{ name: "description", content: "Extract text from PDF for free. Convert PDF to plain text file. Copy or download extracted content. Works 100% in your browser." },
			{ name: "keywords", content: "pdf to text, extract text from pdf, pdf text extractor, convert pdf to txt, pdf to txt" },
			{ property: "og:title", content: "PDF to Text Free - Extract Text from PDF" },
			{ property: "og:description", content: "Extract all text from PDFs. Copy or download as .txt file. Works 100% offline." },
		],
	}),
  ...
  const processFile = useCallback(
    async (fileToProcess: File) => {
      ...
      try {
        ...
        setError(getErrorMessage(err, "Failed to extract text"));
      } finally {
        stopProcessing();
      }
    },
