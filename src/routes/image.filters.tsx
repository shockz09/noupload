...
export const Route = createFileRoute("/image/filters")({
	head: () => ({
		meta: [
			{ title: "Image Filters Free - Apply Photo Effects Online | noupload" },
			{ name: "description", content: "Apply filters to images for free. Grayscale, sepia, invert, vintage, and artistic effects. Works offline, completely private." },
			{ name: "keywords", content: "image filters, photo filters, photo effects, grayscale filter, sepia filter, free image filters" },
			{ property: "og:title", content: "Image Filters Free - Apply Photo Effects Online" },
			{ property: "og:description", content: "Apply beautiful filters to images for free. Works 100% offline." },
		],
	}),
	component: ImageFiltersPage,
});
  ...
  if (filter === "glitch") {
    ...
    const getColor = (t: number) => {
      for (let j = 0; j < THERMAL_GRADIENT.length - 1; j++) {
        const c1 = THERMAL_GRADIENT[j],
          c2 = THERMAL_GRADIENT[j + 1];
...
function ImageFiltersPage() {
  ...
  const previewSrc = useMemo(
    () => (isPixelBased && livePreview ? livePreview : preview),
    [isPixelBased, livePreview, preview],
  );
            ...
            >
              {isProcessing ? (
                ...
                <>
                  ...
                  {selectedFilter ? `Apply ${filterLabel}` : "Select a Filter"}
                </>
              )}
            </button>
          </div>
