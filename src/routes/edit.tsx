...
export const Route = createFileRoute("/edit")({
	head: () => ({
		meta: [
			{ title: "Edit PDF Free - Add Text, Images & Annotations Online | noupload" },
			{ name: "description", content: "Edit PDF files for free. Add text, images, signatures, highlights, and annotations. Draw, redact, and stamp PDFs. Works offline, completely private." },
			{ name: "keywords", content: "edit pdf, pdf editor, annotate pdf, add text to pdf, pdf markup, free pdf editor, online pdf editor" },
			{ property: "og:title", content: "Edit PDF Free - Add Text, Images & Annotations Online" },
			{ property: "og:description", content: "Edit PDF files for free. Add text, images, signatures, and annotations. Works 100% offline." },
		],
	}),
...
import { RedactionConfirmDialog } from "@/app/edit/components/RedactionConfirmDialog";
import { SignatureModal } from "@/app/edit/components/SignatureModal";
import { ZoomControls } from "@/app/edit/components/ZoomControls";
import { useAutoSave } from "@/app/edit/hooks/useAutoSave";
import type { FormField } from "@/app/edit/hooks/useFormFields";
import type { EditorObjectRecord } from "@/app/edit/lib/editor-objects";
import { countRedactions, exportPdf } from "@/app/edit/lib/export-pdf";
...
function EditPdfPage() {
  ...
  const handleUndoRedoChange = useCallback(
    (newCanUndo: boolean, newCanRedo: boolean, undoFn: () => void, redoFn: () => void) => {
