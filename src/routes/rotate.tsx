...
export const Route = createFileRoute("/rotate")({
	head: () => ({
		meta: [
			{ title: "Rotate PDF Online Free - Rotate PDF Pages | noupload" },
			{ name: "description", content: "Rotate PDF pages 90, 180, or 270 degrees for free. Rotate individual pages or entire document. Works offline, no uploads needed." },
			{ name: "keywords", content: "rotate pdf, rotate pdf pages, pdf rotation, turn pdf, flip pdf, rotate pdf online" },
			{ property: "og:title", content: "Rotate PDF Online Free - Rotate PDF Pages" },
			{ property: "og:description", content: "Rotate PDF pages for free. Works 100% offline, no uploads needed." },
		],
	}),
  ...
  const handleRotate = useCallback(async () => {
    ...
    try {
      ...
      for (const [rotation, pageNumbers] of Object.entries(rotationGroups)) {
        const rotated = await rotatePDF(currentFile, parseInt(rotation, 10) as 0 | 90 | 180 | 270, pageNumbers);
