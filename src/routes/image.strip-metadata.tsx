...
export const Route = createFileRoute("/image/strip-metadata")({
	head: () => ({
		meta: [
			{ title: "Image Metadata Viewer & Remover - View & Strip EXIF Data Free | noupload" },
			{ name: "description", content: "View and remove image metadata for free. Inspect EXIF data, GPS location, camera info, then strip it all. Protect your privacy. Works offline." },
			{ name: "keywords", content: "image metadata viewer, view exif data, remove image metadata, strip exif, remove exif data, image privacy, remove gps from photo, free metadata remover, exif viewer, photo metadata" },
			{ property: "og:title", content: "Image Metadata Viewer & Remover - View & Strip EXIF Data Free" },
			{ property: "og:description", content: "View and remove image metadata and EXIF data for free. Inspect camera info, GPS location, then strip it all. Works 100% offline." },
		],
	}),
	component: StripMetadataPage,
});
