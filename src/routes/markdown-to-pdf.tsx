...
export const Route = createFileRoute("/markdown-to-pdf")({
	head: () => ({
		meta: [
			{ title: "Markdown to PDF Converter Free - Export MD with Math Support | noupload" },
			{ name: "description", content: "Convert Markdown to PDF for free. Full LaTeX math equation support, syntax highlighting, tables, and more. Live preview. Works offline, completely private." },
			{ name: "keywords", content: "markdown to pdf, convert markdown to pdf, md to pdf, markdown pdf converter, latex to pdf, markdown math pdf, free markdown converter" },
			{ property: "og:title", content: "Markdown to PDF Converter Free - Export MD with Math Support" },
			{ property: "og:description", content: "Convert Markdown to PDF with LaTeX math support. Live preview. Works 100% offline." },
		],
	}),
	component: MarkdownToPdfPage,
});
...
async function renderMarkdownWithMath(markdown: string): Promise<string> {
  const [katex, marked, DOMPurify] = await Promise.all([getKatex(), getMarked(), getDOMPurify()]);
  ...
  return (
    <button
      ...
      className={`w-8 h-8 flex-shrink-0 flex items-center justify-center rounded transition-colors ${
        active ? "bg-primary/20 text-primary" : "hover:bg-muted text-foreground/70 hover:text-foreground"
      }`}
    >
      {children}
    </button>
    ...
    <main className="container py-8 max-w-7xl">
      ...
      <div className="mt-8 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {lastSaved && (
            <span
              className={`text-xs hidden sm:flex items-center gap-1 transition-colors duration-300 ${saveAnimation ? "text-green-500" : "text-muted-foreground"}`}
            >
              {saveAnimation && (
        ...
        <div className="flex flex-wrap items-center gap-2 p-3 bg-card border border-foreground/10 rounded-lg">
                      ...
                      {THEMES.map((t) => (
                        <button
                          ...
                          className={`flex-1 px-3 py-2 text-xs font-medium rounded transition-all ${
                            theme === t.id ? "ring-2 ring-primary ring-offset-2" : "hover:bg-muted"
                          }`}
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
            <div
              className={`relative w-10 h-5 rounded-full transition-colors ${showPreview ? "bg-primary" : "bg-muted"}`}
            >
              <div
                className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${showPreview ? "translate-x-5" : "translate-x-0.5"}`}
              />
              <input
                type="checkbox"
                checked={showPreview}
        ...
        {/* Editor and Preview */}
        <div
          className={`grid gap-4 overflow-hidden ${showPreview ? "lg:grid-cols-2" : ""}`}
          style={{ height: "calc(100vh - 280px)", minHeight: "400px" }}
        >
          {/* Editor Panel */}
          <div
            className="flex flex-col border border-foreground/10 rounded-lg bg-card focus-within:outline-none h-full min-h-0 overflow-hidden"
            style={{ outline: "none" }}
          >
            {/* Formatting Toolbar */}
            <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-foreground/10 bg-muted/30">
              ...
              <div className="relative" ref={emojiRef}>
                <ToolbarButton onClick={() => setShowEmoji(!showEmoji)} title="Insert Emoji" active={showEmoji}>
              ...
              <div className="relative" ref={tocRef}>
                <ToolbarButton onClick={() => setShowToc(!showToc)} title="Table of Contents" active={showToc}>
                  <svg
                    aria-hidden="true"
                    className="w-4 h-4"
                    viewBox="0 0 24 24"
                  ...
                  <div className="absolute top-full right-0 mt-2 p-2 bg-card border border-foreground/10 rounded-lg shadow-xl z-[100] w-64 max-h-80 overflow-y-auto">
                    ...
                    {tableOfContents.length === 0 ? (
                      ...
                      tableOfContents.map((item) => (
                        ...
                        >
                          <span className="text-muted-foreground mr-1.5">{"#".repeat(item.level)}</span>
                          {item.text}
                        </button>
                      ))
                    )}
                ...
                <div className="flex items-center gap-2 ml-auto">
                  <button
                    ...
                    className={`flex items-center gap-1 px-2 py-0.5 text-xs rounded transition-colors ${
                      scrollSync ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted"
                    }`}
