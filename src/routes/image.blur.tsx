...
export const Route = createFileRoute("/image/blur")({
	head: () => ({
		meta: [
			{ title: "Blur Image Free - Add Blur Effect Online | noupload" },
			{ name: "description", content: "Add blur effect to images for free. Gaussian blur, motion blur, and more. Adjust blur intensity. Works offline, completely private." },
			{ name: "keywords", content: "blur image, add blur, image blur effect, gaussian blur, blur photo, free blur tool" },
			{ property: "og:title", content: "Blur Image Free - Add Blur Effect Online" },
			{ property: "og:description", content: "Add blur effects to images for free. Works 100% offline." },
		],
	}),
	component: ImageBlurPage,
});
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2">
              ...
              >
                {preview && <img src={preview} alt="Preview" className="w-full h-auto block" draggable={false} />}
                <canvas
                  ref={canvasRef}
              ...
              <p className="text-xs text-muted-foreground mt-2">Click and drag to select areas to blur</p>
            </div>
                ...
                {regions.length === 0 ? (
                  ...
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {regions.map((region, i) => (
                      <div key={region.id} className="flex items-center justify-between py-1 px-2 bg-muted/50 text-sm">
                        <span>Area {i + 1}</span>
                        <button
                          type="button"
          ...
          >
            ...
            {isProcessing
              ? "Processing..."
              : `Apply ${blurType === "pixelate" ? "Pixelate" : "Blur"} to ${regions.length} Area${regions.length !== 1 ? "s" : ""}`}
          </button>
        </div>
      )}
    </div>
