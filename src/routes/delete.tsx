...
export const Route = createFileRoute("/delete")({
	head: () => ({
		meta: [
			{ title: "Delete PDF Pages Free - Remove Pages Online | noupload" },
			{ name: "description", content: "Delete pages from PDF for free. Select and remove unwanted pages from your document. Works 100% in your browser." },
			{ name: "keywords", content: "delete pdf pages, remove pdf pages, trim pdf, cut pdf pages" },
			{ property: "og:title", content: "Delete PDF Pages Free" },
			{ property: "og:description", content: "Select and delete pages from your PDF. Works 100% offline." },
		],
	}),
...
function DeletePage() {
  ...
  const toggleDelete = useCallback((id: string) => {
    setPageItems((prev) => prev.map((p) => (p.id === id ? { ...p, deleted: !p.deleted } : p)));
  }, []);
            ...
            <div className="grid sm:grid-cols-3 gap-4 text-sm">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">Mark pages to remove</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 border-2 border-foreground flex items-center justify-center shrink-0 text-xs font-bold">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">Get cleaned PDF</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          {pagesLoading ? (
            ...
            <>
                ...
                <div className="flex items-center gap-3 text-sm flex-wrap">
                  ...
                  {deletedCount > 0 && (
                    <>
                      ...
                      <span className="text-red-500 font-bold">{deletedCount} deleted</span>
                    </>
                  )}
                  {rotatedCount > 0 && (
                    <>
                      ...
                      >
                        ...
                        >
                          <span className="font-mono">
                            {item.deleted ? "×" : index + 1 - pageItems.slice(0, index).filter((p) => p.deleted).length}
                          </span>
              ...
              {!hasChanges && (
                <p className="text-center text-sm text-muted-foreground">Click on pages to mark them for deletion</p>
              )}
            </>
          )}
        </div>
